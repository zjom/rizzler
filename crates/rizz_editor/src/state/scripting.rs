...
//!
//! The runtime is moved out of `Scripting` for the duration of an eval (so
//! editor primitives can mutate `State` through the thread-local bridge
//! without aliasing `self.scripting.lisp`). Notifications fired by Rust
//! during an outer eval are queued and drained on the way out — see
//! [`State::with_lisp`].
...
/// Lisp runtime plus the per-conversation notification queue. `lisp` is
/// `Option` so [`State::with_lisp`] can `take` it for the duration of an
/// eval (the editor-side bridge mutates `State` while the runtime is held
/// elsewhere). Notifications that fire while the runtime is checked out
/// land in `pending_notifications` and drain on the way out.
pub(super) struct Scripting {
    pub lisp: Option<LispRuntime>,
...
impl State {
    ...
    /// that `self.scripting.lisp` is owned again. A cap keeps a buggy
    /// `notify` definition from looping forever — anything past the cap
    /// falls back to the message journal so it's still recoverable via
    /// `:messages`.
    pub(super) fn drain_pending_notifications(&mut self) {
        const MAX_DRAIN_PER_CALL: usize = 32;
        let mut drained = 0;
        while let Some(msg) = self.scripting.pending_notifications.pop() {
            if drained >= MAX_DRAIN_PER_CALL {
                warn!(
                    remaining = self.scripting.pending_notifications.len() + 1,
                    "notification drain cap hit — recording remainder to journal"
                );
                let remainder: Vec<String> = std::iter::once(msg)
                    .chain(self.scripting.pending_notifications.drain(..))
                    .collect();
                for m in remainder {
    ...
    pub fn notify_via_lisp(&mut self, msg: &str) {
        ...
        if self.scripting.lisp.is_none() {
            debug!("notify_via_lisp queued — lisp runtime checked out");
            self.scripting.pending_notifications.push(msg.to_string());
            return;
        }
