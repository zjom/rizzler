...
impl BufferList {
    ...
    /// popups). Returns `true` iff something was removed.
    ...
    /// Locate a file buffer matching `path`, if any.
    ...
    /// Used by `:badd` / `:edit` to dedup against already-open files.
    pub fn buffer_for_path(&self, path: Rc<Path>) -> Buffer {
        self.bufs
            .iter()
            .find(|b| b.fs_path().as_deref() == Some(path.as_ref()))
            .cloned()
            .unwrap_or_else(|| crate::buffer_io::with_path(path))
    }

    /// How many non-minibuffer buffers exist.
